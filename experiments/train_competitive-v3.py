...
def get_trainers(env, obs_shape_n, arglist):
    ...
    for i in range(env.n):
        trainers.append(trainer(
            "agent_%d" % i, model, [obs_shape_n[i]], [env.action_space[i]], i, arglist,
    ...
    with U.single_threaded_session():
        ...
        while True:
            ...
            for agent in trainers:
                agent.preupdate()
            for agent in trainers:
                loss = agent.update([agent], train_step)
            ...
            if len(episode_rewards_own) > arglist.num_episodes:
                ...
                file_name = 'Models/history_' + scenario_name + '_split_' + str(cnt) + '.pkl'
                with open(file_name, 'wb') as fp:
                    pickle.dump(hist, fp)
                print('...Finished total of {} episodes.'.format(len(episode_rewards_own)))
                file_model = 'Models/' + scenario_name + '_split_' + str(cnt) + '.pkl'
